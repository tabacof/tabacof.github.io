...
Evaluation is the framework that objectively assesses how well a model performs. The evaluation process should closely resemble the expected outcomes after the model's deployment. While the standard train-test or cross-validation splits provide a starting point, they are generally insufficient. If the problem evolves over time (as is the case for nearly all business problems), a [time-series split](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.TimeSeriesSplit.html) should be employed. If the target is censored for six months, there should be a six-month gap between training and testing. If the data contains groups and the model will make predictions for new groups in the future, you should use a [group split](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.GroupKFold.html).
...
This blog post originally appeared in the [Data Hackers Medium](https://medium.com/data-hackers/a-hierarquia-de-necessidades-de-machine-learning-813a66369b83) in 2019. Full disclosue: the translation was done with the help of GPT4. I reviewed the final text and updated some sections, since I've personally learned and grown a lot since 2019. I'm open to feedback or suggestions for more applications of the hierarchy of needs framework.